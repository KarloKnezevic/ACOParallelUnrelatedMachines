...
typedef struct{
	...
	int rb_stroj;					//redni broj stroja koji obraðuje posao
...
//vektor za praæenje prioriteta na stroju prilikom ACO rasporeðivanja na zahtjev 
...
Q je potreban da bi mrav odluèio hoæe li odlazak u novu aktivnost biti donesena na temelju vjerojatnosne funkcije ("istraživanje")
...
void inicijalizacija_poslova(int N){
	...
	//odreðivanje poèetnog položaja mrava
	//mravi se rasporeðuju kružno, po modulu aktivnosti
	vector<int> pom3;
	int mche=-1;
	int mch;
...
Korisnik odreðuje inicijalnu vrijednost tau.
...
Mrav je nauèio da odlazi u onu aktivnost kojoj je umnožak feromonskog traga do te aktivnosti i njezine heuristièke vrijednsoti najveæa.
...
int razlika_vremena(vector<posao> neprocesuirani,int trenutno_vrijeme,vector<int> gotove,int max_prioritet){
	...
	//potraži sve koji èiji su prethodnici završili U NEPROCESUIRANIM!
	for(int i=0;i<neprocesuirani.size();i++){
		...
		//ne treba se tražiti poslove èiji prioritet je veæi max_prioritet+1 
		if(neprocesuirani[i].prioritet>(max_prioritet+1))
			break;
	}

	//potraži najmanju pripravnost kod poslova èiji prethodnici su završili jer taj posao zapoèinje prvi od nezapoèetih
	if(zavrseni.size()!=0){
...
AKO JE 1, TADA GA KORISTI ACO, AKO JE 0, KORISTI GA MIN_MIN,  A AKO JE -1, TADA GA KORISTI ACO S RASPOREÐIVANJEM NA ZAHTJEV  */
...
label1:		if(!pokrenute.empty()){
			...
			if(trenutno_vrijeme<max_pripravnost){
				//racuna razliku vremena izmeðu prvog posla èiji prethodnik je završio, a èeka da postane pripravan i pokrenutog posla èije trajanje je minimalno
	...
	//fitness može vraæati 4 moguæe vrijednosti: Cmax / Fw / Tw / Uw
...
Ne èita se cijela datoteka odjednom, nego se datoteka èite red po red kako se informacije obraðuju
...
/*Metoda za citanje trajanja poslova na odreðenosm stroju
...
/*Metoda za èitanje težinskih faktora poslova
citam N+1 red, pocevsi brojati od 0. */
...
/********************************************MIN-MIN********************************************/
/*MIN_MIN algoritam za PRIDRUŽIVANJE I UREÐIVANJE poslova
...
void min_min(int N){
	...
	while((dolazeci.size()>0) || (neprocesuirani.size()>0)|| (Jmeta.size()>0) ){
		...
		//ovdje se vrsi pridruzivanje i ureðivanje -> koji posao se radi na kojem stroju i koji je po redu u redoslijedu obrade
		while(!Jmeta.empty()){
			...
			indeks_stroj=1;
			indeks_posao=0;
			//pronaði najmanji Cij u cijeloj matrici
			for(int i=0;i<Jmeta.size();i++){
				for(int j=0;j<vektor_strojeva[N];j++){
					if(C[i][j]<min_c){
			...
			//dodjeljivanje posla stroju i ureðivanje 
		...
		for(int i=0;i<vektor_strojeva[N];i++){
			...
			if((zaposlen[i]==0)&&(neprocesuirani.size()!=0)){
				//pronaði posao èiji prioritet izvoðenja na tom stroju je najmanji
				...
				//kad je posao pronaðen, on se pokreæe na tom stroju, odnosno, briše se posao iz neprocesuranih, a poveæava se vrijeme priprave stroja i za trajanje posla j
...
/*ACO algoritam za PRIDRUŽIVANJE I UREÐIVANJE poslova*/
void ACO(int N){
	...
	for(int i=0;i<broj_generacija_mrava;i++){
		...
		for(int i=0;i<vektor_poslova[N];i++){
				...
				if(i<vektor_poslova[N]-1){
					...
					for(int k=0;k<broj_mrava;k++){
						q=drand();
						//odreðivanje hoæe li mrav odabirati odlazak na temelju vjerojatnosne funkcije ili na temelju nauèenog			
						if(q<=q0){
			...
			for(int j=0;j<vektor_poslova[N];j++){
				for(int k=0;k<vektor_strojeva[N];k++){
					...
					if(heuristicke_vrijednosti[i][j][k]!=Beskonacnost){
						...
						//time se postiže toènija konvergencija
...
/*Obzirom na poslove u Jmeta, mrave rasporeðuje u aktivna podruèja*/
...
/*ACO algoritam za PRIDRUŽIVANJE I UREÐIVANJE NA ZAHTJEV*/
			...
			if((zaposlen[i]==0)&&(najbolji_slijed_aktivnosti.size()!=0)){
				...
				for(int j=0;j<najbolji_slijed_aktivnosti.size();j++){
					if(najbolji_slijed_aktivnosti[j][1]-1==i){
						pokrecem_posao=j;
						//poveæaj prioritet izvoðenja na tom stroju
						prati_prioritete[i]++;;
				...
				//kad je posao pronaðen, on se pokreæe na tom stroju, odnosno, briše se posao iz najbolji_slijed_aktivnosti, a poveæava se vrijeme priprave stroja i za trajanje posla j
				for(int k=0;k<poslovi.size();k++){
